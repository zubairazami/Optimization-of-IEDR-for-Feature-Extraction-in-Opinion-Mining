...
class SetupWidget(QtWidgets.QWidget):
    def __init__(self, interaction_data):
        ...
        self.pos_file_text_area = QtWidgets.QTextBrowser(self)
        self.pickled_directory_text_area = QtWidgets.QTextBrowser(self)
        self.neg_file_text_area = QtWidgets.QTextBrowser(self)
    ...
    def setup_ui(self):
        self.pos_file_text_area.setGeometry(QtCore.QRect(240, 150, 501, 31))
        self.pickled_directory_text_area.setGeometry(QtCore.QRect(240, 270, 501, 31))
        self.neg_file_text_area.setGeometry(QtCore.QRect(240, 210, 501, 31))
    ...
    def button_action(self):
        ...
        if sender == self.pscf_pushbutton:
            file = str(QtWidgets.QFileDialog.getOpenFileName(self, "Select Positive Sentiment containing file",
                                                         expanduser('~') + "/dataset"))
            self.pos_file_text_area.setText(file)
        ...
        if sender == self.nscf_pushbutton:
            file = str(QtWidgets.QFileDialog.getOpenFileName(self, "Select Negative Sentiment containing file",
                                                         expanduser('~') + "/dataset"))
            self.neg_file_text_area.setText(file)

        if sender == self.pfd_pushbutton:
            file = str(QtWidgets.QFileDialog.getExistingDirectory(None, "Select Pickled files directory",
                                                              expanduser('~') + "/dataset"))
            self.pickled_directory_text_area.setText(file)

        if self.interaction_data.new_thread_allowable:
            if sender == self.set_pushbutton:
                pos = self.pos_file_text_area.toPlainText()
                neg = self.neg_file_text_area.toPlainText()
                pickled = self.pickled_directory_text_area.toPlainText()
    ...
    def button_click_action(self):
        if self.interaction_data.is_set_sentiment_dictionary:
            if self.interaction_data.new_thread_allowable:
                ...
                self.training_thread.trainingSignal.connect(self.update_progress_1)
                self.training_thread.trainingClassifierSignal.connect(self.update_progress_2)
                self.training_thread.finishSignal.connect(self.on_completion)
            ...
            if self.interaction_data.new_thread_allowable:
                ...
                if sender == self.analysis_button:
                    ...
                    if len(test_file) == 0 or len(feature_file) == 0:
                        ...
                        self.analysis_thread.analysisSignal.connect(self.update)
                        self.analysis_thread.finishSignal.connect(self.on_completion)
                        self.interaction_data.deny_new_thread()
