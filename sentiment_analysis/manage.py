...
class SentimentManager(object):
    ...
    def train(self):
        ...
        training_obj = Training(
            positive_file_directory=self.positive_file_directory,
        ...
        self.emitter.finishSignal.emit()

    def result(self, test_file_path, corpus_feature_file_path):
        if self.training_obj:
            self.training_obj = None
        self.training_obj = Training(
            positive_file_directory=self.positive_file_directory,
            negative_file_directory=self.negative_file_directory,
            pickled_directory=self.pickled_directory
        )
        ...
        if self.testing_obj:
            self.testing_obj = None
        self.testing_obj = SentimentTesting(
            training_object=self.training_obj,
            analysed_file_path=test_file_path,
            corpus_feature_file_path=corpus_feature_file_path,
